...
static bool check_call_from_hfi_process_msg_packet(void)
{
	bool result = false;
	...
	frame.sp = current_sp;
	frame.pc = (unsigned long) check_call_from_hfi_process_msg_packet;
	...
	while (1) {
		...
		if (ret < 0)
			break;
		snprintf(stack, sizeof(stack) - 1, "%pS", (void *) where);
		stack[sizeof(stack) - 1] = '\0';
		if (strncmp(stack, "hfi_process_msg_packet+", 23) == 0) {
			result = true;
		}
	}
...
static int venus_hfi_session_clean(void *session)
{
	...
	dprintk(VIDC_DBG, "deleted the session: 0x%pK\n",
			sess_close);
	if (check_call_from_hfi_process_msg_packet() && mutex_is_locked(&device->session_lock)) {
		dprintk(VIDC_ERR, "%s: Called from hfi_process_msg_packet\n", __func__);
		should_lock = false;
	}
	if (likely(should_lock)) {
		mutex_lock(&device->session_lock);
	}
